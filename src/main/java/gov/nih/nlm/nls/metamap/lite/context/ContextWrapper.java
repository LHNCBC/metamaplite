...
/**
 *
 * method applyContext returns result 
 */
...
public class ContextWrapper {
  ...
  public static List<List<String>> applyContextUsingEntities(List<Entity> entityList, 
    ...
    for (Entity entity: entityList) {
      for (String matchedWord: entity.getMatchedWordSet()) {
	for (Sentence sentence: sentenceList) {
	  resultlist.add(contextInstance.applyContext(matchedWord, sentence.getText()));
	}
      }
    }
    return resultlist;
  ...
  /** given annotated sentence with entities, determine hedging relations using context.*/
  public static BioCSentence applyContext(BioCSentence sentence) throws Exception {
	...
	List<String> result = 
	  contextInstance.applyContext(((BioCEntity)annotation).getEntity().getMatchedText(), 
				       sentence.getText());
	if (result != null) {
